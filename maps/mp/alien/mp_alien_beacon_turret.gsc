...
play_turret_fx( var_0 )
{
    ...
    {
        if ( self.off )
            killfxontag( level._ID1644["glow_red_light_100_nolight"], self, "tag_light_fx" );
        else
            killfxontag( level._ID1644["green_light"], self, "tag_light_fx" );

        wait 0.5;

        if ( self.overloaded )
        {
            playfxontag( level._ID1644["boss_turret_smoke"], self, "TAG_PLAYER" );
            return;
        }

        playfxontag( level._ID1644["glow_red_light_100_nolight"], self, "tag_light_fx" );
        return;
        return;
    }
    ...
    if ( self.overloaded )
    {
        stopfxontag( level._ID1644["boss_turret_smoke"], self, "TAG_PLAYER" );
        wait 5;
    }
    else
    {
        stopfxontag( level._ID1644["glow_red_light_100_nolight"], self, "tag_light_fx" );
        wait 0.5;
    }

    if ( !self.off )
    {
        playfxontag( level._ID1644["green_light"], self, "tag_light_fx" );
        return;
    }
